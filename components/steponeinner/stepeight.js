 ...
 useEffect(() => {
  ...
  const handleFileChange = async (event) => {
    ...
    if (file.type === "application/pdf") {
      ...
      seteinkommensbescheinigungimgshow(file); // Show first page preview
    } else {
      updatedImages = [file];
            ...
            <div className="flex flex-col w-full justify-center mx-auto">
              ...
              {einkommensbescheinigungimgshow && !isConverting && (
                <div className="relative w-24 h-24">
                  {einkommensbescheinigungimgshow instanceof File &&
                  einkommensbescheinigungimgshow.type.startsWith("image/") ? (
                    <img
                      src={URL.createObjectURL(einkommensbescheinigungimgshow)}
